...
import re
from district_match.fj_api.matchApi import match_api
from lib.standardization import StandardCity as sc
from district_match.format_tools.format_name import format_name_address
...
class MatchSeaweed:
    ...
    def seaweed_name_list(cls, match_info_dict):
        ...
        # 加入地址别名
        addalias = cls.get_address_alias(match_info_dict['city'], match_info_dict['district'], match_info_dict['name'], pattern)
        if addalias:
            name_list += addalias
...
if __name__ == '__main__':
    print(match([], **{'region':'长宁','keyword': '文洋大厦', 'category': 'office'}))
