...
log = LogHandler('baidumap_consumer_update')
"""
字典格式:
dict = {
"city":"",
...
class BaiduMapConsumer:
    ...
    @retry(logger=log.error('获取总页数的请求失败'),delay=2)
    def send_first_url(self, address_dict):
        ...
        try:
            ...
            log.error('{}的错误是:'.format(rest.url),e)
            return
        if total == 0:
    ...
    def send_page_url(self, address_dict, total_pages):
        ...
        for page_num in range(total_pages + 1):
            ...
            res,rest = self.send_page(payload)
            self.analyse_detail(res, address_dict, rest)

    @retry(logger=log.error('请求具体的页数的时候失败'),delay=2)
